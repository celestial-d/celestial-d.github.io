...
My current research focuses on leveraging emerging hardware technologies, such as **Compute Express Link (CXL)**, to design scalable memory and **storage systems** for **AI infrastructure**. My research particularly explores KV cache storage, shared memory pooling, and unified storage abstractions for AI workloads, with an emphasis on improving efficiency and scalability in high-performance computing (HPC) environments.
